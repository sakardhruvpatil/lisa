...
input_clean_log_filename = f"input_clean_{timestamp}.txt"
...
starting_edge_active, ending_edge_active, bedsheet_processing_active = (
    False,
    False,
    False,
)
...
# Flag to indicate that defect percent has been calculated and is being displayed
defect_percent_active = False
...
while cap.isOpened():
            ...
            for idx, class_id in enumerate(classes):
                if int(class_id) == 0 and confidences[idx] > conf_threshold:
                    ...
                    if tracking_active and y2 < previous_y2:
                        ...
                        new_area = (
                            new_segment_height * box_width
                        )  # Use the bounding box width for area calculation
                        total_bedsheet_area += new_area
                        log_print(
                            f"Added new area segment: Height = {new_segment_height}, Width = {box_width}, Area = {new_area}"
                        )
                        log_file.write(
                            f"Bedsheet {bedsheet_count + 1}: Added new area segment: Height = {new_segment_height}, Width = {box_width}, Area = {new_area}\n"
                        )
                        log_file.flush()
    ...
    if bedsheet_present and y1_positions and y2_positions:
        ...
        if not bedsheet_processing_active and y1_min > frame_height * 0.75:
            starting_edge_active, area_accumulating, bedsheet_processing_active = (
                True,
                True,
                True,
            )
        ...
        if tracking_active and not ending_edge_active and y2_max < frame_height * 0.90:
            ...
            log_print(
                f"Ending Edge Detected. y2_max: {y2_max}, frame_height: {frame_height}"
            )
            log_file.write(
                f"Ending Edge Detected. y2_max: {y2_max}, frame_height: {frame_height}\n"
            )
            ...
            log_print(
                f"Total Bedsheet Area at Ending Edge Appearance: {total_bedsheet_area}"
            )
            ...
            analysis_message = (
                ...
                f"Bedsheet {bedsheet_count + 1}: {cleanliness_status}. "
                f"Dirty Percent: {defect_percent:.2f}%, "
            ...
            log_print(
                f"Bedsheet {bedsheet_count + 1}: Defect Percent = {defect_percent:.2f}%"
            )
            log_print(
                f"Bedsheet {bedsheet_count + 1}: Clean Percent = {clean_percent:.2f}%"
            )
            total_defect_area_log_file.write(
                f"Bedsheet {bedsheet_count + 1}: Total Defect Area = {total_defect_area}\n"
            )
            defect_percent_log_file.write(
                f"Bedsheet {bedsheet_count + 1}: Total Bedsheet Area = {total_bedsheet_area}, "
                f"Total Defect Area = {total_defect_area}, Defect Percent = {defect_percent:.2f}%, "
                f"Clean Percent = {clean_percent:.2f}%\n"
            )
            ...
            # Activate defect percent display
            defect_percent_active = True
            ...
            log_file.write(
                f"Bedsheet {bedsheet_count}: Ending Edge Detected and Counted\n"
            )
            log_file.flush()
            ...
            for defect_result in defect_results:
                ...
                if masks is not None and tracks is not None:
                    ...
                    for j, mask in enumerate(mask_array):
                        ...
                        cv2.rectangle(
                            frame_resized, (x1_d, y1_d), (x2_d, y2_d), (0, 0, 255), 2
                        )
                        log_print(
