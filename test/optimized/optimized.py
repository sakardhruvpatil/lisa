...
# Use PyMongoError for generic exceptions
...
# Define at the top if not already
local_timezone = pytz.timezone("Asia/Kolkata")
...
def update_threshold(val):
    ...
    if threshold_changed:
        ...
        initialize_history_document(
            get_current_date_str(), CLEAN_THRESHOLD
        )  # Initialize history only on change
...
cv2.createTrackbar(
    "Clean Threshold",
    "Video with FPS and Detection Status",
    int(CLEAN_THRESHOLD),
    100,
    update_threshold,
)
...
# Reset defect tracking variables
                        ...
                        if masks is not None and tracks is not None:
                            ...
                            for j, mask in enumerate(mask_array):
                                ...
                                if defect_percent_real_time >= 100:
                                    ...
                                    log_print(
                                        f"Bedsheet {bedsheet_count + 1}: Rejected due to defect percent >= 100%"
                                    )
                                    decision = "Rejected"
                                    log_to_mongo(
                                        bedsheet_count + 1,
