...
while cap.isOpened():
    ...
    if bedsheet_present and y1_positions and y2_positions:
        ...
        if tracking_active and not ending_edge_active and y2_max < frame_height * 0.75:
            ...
            # Log and print the total defect area for the current bedsheet
            log_print(f"Bedsheet {bedsheet_count}: Total Defect Area = {total_defect_area}")
            total_defect_area_log_file.write(f"Bedsheet {bedsheet_count}: Total Defect Area = {total_defect_area}\n")
            total_defect_area_log_file.flush()
