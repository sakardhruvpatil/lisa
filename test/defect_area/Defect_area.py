...
while cap.isOpened():
    ...
    if bedsheet_present and y1_positions and y2_positions:
        ...
        if bedsheet_processing_active and not ending_edge_active and y2_max < frame_height * 0.75:
            ...
            bedsheet_count += 1
            defect_count = len(unique_defect_ids)  # Calculate unique defects for the bedsheet
            log_file.write(f"Bedsheet {bedsheet_count}: Area = {total_bedsheet_area}\n")
            log_file.flush()
            defect_log_file.write(f"Bedsheet {bedsheet_count}: Total Defects = {defect_count}\n")
            defect_log_file.flush()
            log_print(f"Bedsheet {bedsheet_count}: Area = {total_bedsheet_area}")
            log_print(f"Bedsheet {bedsheet_count}: Total Defects = {defect_count}")
            ...
            for defect_result in defect_results:
                ...
                if masks is not None and tracks is not None:
                    ...
                    for j, mask in enumerate(mask_array):
                        ...
                        # Increment bedsheet_count for logging
                        incremented_bedsheet_count = bedsheet_count + 1

                        # Log the defect coordinates and area
                        x1_d, y1_d, x2_d, y2_d = defect_result.boxes.xyxy[j].int().tolist()
                        
                        # Log the defect coordinates and area with bedsheet ID
                        defect_area_log_file.write(f"Bedsheet {incremented_bedsheet_count} - Defect ID {defect_id}: Coordinates ({x1_d}, {y1_d}, {x2_d}, {y2_d}): Area = {defect_area} pixels\n")
                        defect_coordinates_log_file.write(f"Bedsheet {incremented_bedsheet_count} - Defect ID {defect_id}: Coordinates ({x1_d}, {y1_d}, {x2_d}, {y2_d})\n")

                        cv2.rectangle(frame_resized, (x1_d, y1_d), (x2_d, y2_d), (0, 0, 255), 2)
                        log_print(f"Bedsheet {incremented_bedsheet_count} - Defects Present - Unique ID: {defect_id}: Coordinates ({x1_d}, {y1_d}, {x2_d}, {y2_d}) Area: {defect_area}")
