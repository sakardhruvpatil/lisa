...
const HomeWithWebcam = ({ mode, acceptanceRate, cameraLayout, setLoadingSpeed, loadingSpeed }) => {
    ...
    // State to track bypass mode
    const [isBypassActive, setIsBypassActive] = useState(false);

    // Function to toggle bypass mode
    const toggleBypassMode = async () => {
        try {
            const response = await fetch('http://localhost:8000/toggle-bypass', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ bypass: !isBypassActive ? 1 : 0 }),
            });
            if (!response.ok) throw new Error('Failed to toggle bypass mode');
            setIsBypassActive(!isBypassActive); // Toggle the bypass mode state
        } catch (error) {
            console.error('Error:', error);
            alert('Failed to toggle bypass mode: ' + error.message);
        }
    };
    ...
    return (
        <div className="dashboard">
            ...
            {/* Bypass Button */}
            <div className="bypass-container" style={{ marginTop: '20px' }}>
                <button onClick={toggleBypassMode}>
                    {isBypassActive ? 'Stop Bypass' : 'Bypass Bedsheet'}
                </button>
            </div>
        </div>
    );
