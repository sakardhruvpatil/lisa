...
def connect_modbus():
    ...
    while not client.connect():
        error_code=1019
        log_bug(f"Failed to connect to Modbus server. Retrying... (Error code: {error_code})")
    ...
    if os.path.exists(filepath):
        with open(filepath, 'r') as f:
            ...
            if signal in ['0', '1']:
                ...
                error_code=1020
                log_bug(f"Invalid signal value in {filename}. Expected '0' or '1', got '{signal}'. (Error code: {error_code})")
    ...
    try:
        while True:
            ...
            if not client.is_socket_open():
                error_code=1022
                log_bug(f"Lost connection to Modbus server. Reconnecting... (Error code: {error_code})")
